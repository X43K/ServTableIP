...
def infer_type_from_vendor(vendor, mac):
    """
    Devuelve (tipo, icono, color, host_name, fabricante)
    - host_name: nombre amigable para mostrar en la columna 'Host' (si macs.json tiene 'nombre', se usa aquí)
    - fabricante: valor que debe mostrarse en la columna 'Fabricante' (siempre se obtiene de vendor/OUI o de reglas)
    """
    mac_l = (mac or "").lower()
    macs = cargar_macs()
    # Si hay una entrada exacta en macs.json, usar su nombre como host (pero NO como fabricante)
    if mac_l and mac_l in macs:
        ...
        # fabricante preferentemente del vendor (OUI) si existe, si no usar tipo/regla
    ...
    for regla in tipos:
        ...
        for palabra in palabras:
            if palabra and (palabra in v or palabra in mac_l):
                # fabricante = vendor (si viene) o la descripción de la regla
                fabricante = vendor or regla.get("tipo", "Desconocido")
                # host por defecto lo ponemos igual al fabricante (cuando no hay nombre específico)
    ...
    try:
        ...
        for iface, iface_ip, red in interfaces_disponibles:
            ...
            for ip in (str(h) for h in red.hosts()):
                ...
                # prioridad router .1 (Host seguirá siendo "Router principal",
                # pero "fabricante" se obtiene a partir de la MAC / OUI o de tipos.json)
                if ip.endswith(".1"):
                    mac = tabla_arp_global.get(ip) or mapa_scapy.get(ip) or (mapa_nmap.get(ip)[0] if ip in mapa_nmap else None)
                    if mac:
                        # normalizar la mac para procesar
                        mac_norm = mac.lower()
                    ...
                    # infer_type_from_vendor devuelve (tipo, icono, color, host_name, fabricante)
                    tipo, icono, color, host_name_tmp, fabricante_obtenida = infer_type_from_vendor(vendor, mac_norm)

                    # Queremos mantener el Host mostrado como "Router principal",
                ...
                if mac:
                    # infer_type_from_vendor ahora devuelve host_name y fabricante por separado
                    ...
                    if prefer_current:
                        resultados_por_ip[ip] = {
                            "ip": ip,
                            "mac": mac,
                            # host: prefer name from macs.json (host_name) or vendor-derived name
                            "host": host_name,
                            "tipo": tipo,
                            "icono": icono,
                            "color": color,
                            # fabricante: SIEMPRE vendor/regla (no el 'nombre' de macs.json)
        ...
        # agregar equipo rastreador (local)
        for iface, iface_ip, red in interfaces_disponibles:
            try:
                local_ip = iface_ip
...
# ---------------- Editor HTML (mejorado: dropdown MACs con IPs y lista amplia de emojis) ----------------
EMOJIS_FIXED = [
 "📱","💻","🖥️","🖨️","🔌","🍓","📺","🎧","🎮","📡","🕹️","📷","⌚","🖲️","🛠️","🧭","🚀","🔒","❓"
]
...
function rellenarDesdeSelect(){
  ...
  const v = sel.value;
  if(!v) return;
  // buscar en known_macs (incrustado como JSON en template)
  const known = window.__known_macs || [];
  const item = known.find(x=>x.mac===v);
  if(!item) return;
  // rellenar una nueva fila al inicio con datos
...
def dashboard():
    ...
    <body>
        <header>
            <h1><img src="{{ url_for('static', filename='Logo.webp') }}" alt="Logo" width="35" style="vertical-align: middle;">Dispositivos activos</h1>
...
def editar_tipos():
    tipos = cargar_tipos()
    macs = cargar_macs()
    # build known_macs list from estado + arp table: mac, ip, nombre (if exists), tipo (if exists)
    known = {}
    ...
    # overlay macs persistent values
